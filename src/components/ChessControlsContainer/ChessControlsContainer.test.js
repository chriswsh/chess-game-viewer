...
it(`should pass currentMove and lastMove props to the wrapped component, also called ChessControlsContainer`, () => {
    ...
    expect(rendered.find(`ChessControlsContainer`).props().currentMove).not.toEqual(NaN);
    expect(rendered.find(`ChessControlsContainer`).props().currentMove).not.toEqual(Infinity);
    
    expect(rendered.find(`ChessControlsContainer`).props().lastMove).toEqual(expect.any(Number));
    expect(rendered.find(`ChessControlsContainer`).props().lastMove).not.toEqual(NaN);
    expect(rendered.find(`ChessControlsContainer`).props().currentMove).not.toEqual(Infinity);
});
