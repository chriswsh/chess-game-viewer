...
const mapStateToProps = fullState => {
    ...
    return Object.assign(
        {},
        {
            ...
            lastMove: state.history.length ? state.history.length - 1 : 0
        }
    );
};
...
function ChessControlsContainer(props) {
    const controlList = [
        { move: 0, glyph: `fast-backward` },
        {
            move: props.currentMove ? props.currentMove - 1 : 0,
            glyph: `backward`
        },
        {
            move:
                props.currentMove < props.lastMove
                    ? props.currentMove + 1
                    : props.lastMove,
            glyph: `forward`
        },
...
const ReduxChessControlsContainer = Object.freeze(
    connect(mapStateToProps)(ChessControlsContainer)
);
