...
function ChessButton(props) {
    return (
        <button
            type="button"
            className={`btn ${props.BEMclass}`}
            onClick={() => {
                props.onClick(props.move);
            }}
        >
...
const BEMChessButton = Object.freeze(
    connect(undefined, mapDispatchToProps)(withBEM(ChessButton, `ChessButton`))
);
